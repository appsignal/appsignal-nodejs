...
export function duplicateOpenTelemetryApiWarning(
  ...
  return (
    "More than one version of @opentelemetry/api is loaded. This will cause " +
    "some spans to not be reported:\n" +
    `${listed}\n` +
    "Your application and its dependencies must agree on a single version of " +
    "@opentelemetry/api. Run npm list @opentelemetry/api to see which " +
    "version each of them asks for. If a dependency asks for a version that " +
    "cannot be shared, updating it may be enough. Your package manager can " +
    "also override it to a single version."
  )
}
